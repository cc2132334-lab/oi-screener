...
def generate_executed_trades(symbols, is_fno=True, default_risk_per_trade=1000):
    trade_list = []
    ...
    for idx, sym in enumerate(symbols[:10]):
        ...
        if is_buy:
            ...
            risk_per_share = max(round(entry_price - sl_price, 2), 1.5)
            ...
            risk_per_share = max(round(sl_price - entry_price, 2), 1.5)
            current_ltp = round(entry_price - (risk_per_share * random.uniform(-0.5, 4.5)), 2)
        ...
        if rr_achieved >= 4.0:
            ...
            total_pnl = -round(risk_per_share * qty, 2)
        else:
...
def fetch_market_data():
    ...
    cash_symbols = [
        ...
        "SJVN", "BEML", "KALYANKJIL", "NBCC", "MOTHERSON"
    ]
    ...
    # ALL Breakout stocks sorted descending by volume multiplier
    ...
    print(f"data.json updated at {current_time_str}")
